...
The *atlasgce-modules* provide from-scratch contextualization on bare machines (virtual or physical) for analysis for the [ATLAS Experiment](http://atlas.ch).
...
This contextualization is done from a bare instance meaning that no software other than [Yum](http://yum.baseurl.org/) is required. Even Puppet is installed during the bootstrapping procedure. (Note: This does not apply to CernVM.)
